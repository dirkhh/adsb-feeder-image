...
class TestSystemIntegration:
    ...
    def setup_method(self):
        ...
        # Create rb directory for fake system info
        rb_dir = Path(self.temp_dir) / "rb"
        rb_dir.mkdir()

        # Set ADSB_BASE_DIR to temp directory so all path operations use temp location
        self.old_adsb_base_dir = os.environ.get('ADSB_BASE_DIR')
        os.environ['ADSB_BASE_DIR'] = str(self.temp_dir)

        # Create mock config files
        self.env_file = self.config_dir / ".env"
        self.json_file = self.config_dir / "config.json"

        # Don't mock Path.exists() globally as it breaks pytest's internal error reporting
        # Instead, let tests mock specific paths as needed
    ...
    def teardown_method(self):
        ...
        # Restore ADSB_BASE_DIR environment variable
        if self.old_adsb_base_dir is not None:
            os.environ['ADSB_BASE_DIR'] = self.old_adsb_base_dir
    ...
    def test_config_environment_integration(self):
        ...
        with patch('utils.config.read_values_from_config_json') as mock_read_config:
            ...
            env = data.env_by_tags(["FEEDER_NAME"])
            assert env is not None
            assert env.value == "test_feeder"

            # Test boolean conversion
            enabled = data.is_enabled("ENABLE_SERVICE")
    ...
    def test_config_persistence_integration(self):
        ...
        # Test data
        test_data = {
            "FEEDER_NAME": "integration_test_feeder",
            "FEEDER_LAT": "40.7128",
            "FEEDER_LON": "-74.0060",
            "FEEDER_ALT": "10",
            "ENABLE_SERVICE": "true",
        ...
        with patch('utils.config.read_values_from_config_json') as mock_read, \
            ...
            assert config_values == test_data
...
class TestConfigurationIntegration:
    ...
    def test_config_consistency(self):
        ...
        with patch('utils.config.read_values_from_config_json') as mock_read:
            ...
            # Test boolean conversion
            enabled = data.is_enabled("ENABLE_SERVICE")
            assert enabled is False  # Not in test data
