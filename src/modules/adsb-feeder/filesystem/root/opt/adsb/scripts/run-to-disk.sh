...
if [[ -d /run/acars_data ]] && mkdir -p /opt/adsb/config/acarshub/ && cd /opt/adsb/config/acarshub; then
    ...
    # the rrd file is no longer used by acarshub
    # write the .back file to disk just in case and ignore it from now on
    ...
    elif [[ -f /run/acars_data/acarshub.rrd.back ]]; then
        cp /run/acars_data/acarshub.rrd.back /opt/adsb/config/acarshub/
        rm -f /opt/adsb/config/acarshub/acarshub.rrd.zst
    fi
