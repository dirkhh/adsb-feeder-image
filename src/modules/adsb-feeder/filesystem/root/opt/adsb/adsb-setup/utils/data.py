...
class Data:
    ...
    netconfigs_list = [
        ...
        NetConfig(
            identifier="flightdeck",
            name="FlightDeck",
            website="https://www.tryflightdeck.com/",
            links=[""],
            ordinal=89,
    ...
    _env = {
        ...
        Env("AIRSPY_ADSB_CPUTIME_TARGET", default=60, tags=["airspy_cputime_target"]),
        Env("AIRSPY_ADSB_PREAMBLE_FILTER_MAX", default=20, tags=["airspy_preamble_filter_max"]),
        Env("AIRSPY_ADSB_SAMPLE_RATE", default=12, tags=["airspy_sample_rate"]),
        Env("AIRSPY_ADSB_TIMEOUT", default=90, tags=["airspy_timeout"]),
        ...
        Env("FEED_ACARS_FLIGHTDECK", default=[False], tags=["feed_acars_flightdeck", "is_enabled"]),
    ...
    for name, conf in netconfigs.items():
        # generation / migration for existing uuids happens in app.py handle_implied_settings function
        _env.add(Env(f"ULTRAFEEDER_{name.upper()}_UUID", default=[""], tags=[name, "ultrafeeder", "uuid"]))

        # no migration required as the env var stays the same, it is simply autogenerated from netconfigs now (previously extra code for each agg)
        # special case flyitaly to keep current env var
        if name == "flyitaly":
            env_string = f"_ADSBIM_STATE_IS_ULTRAFEEDER_FLYITALYADSB_ENABLED"
        else:
            env_string = f"_ADSBIM_STATE_IS_ULTRAFEEDER_{name.upper()}_ENABLED"
        _env.add(Env(env_string, default=[False], tags=[name, "ultrafeeder", "is_enabled"]))
