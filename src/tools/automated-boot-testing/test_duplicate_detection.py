...
def app(test_config, tmp_path):
    ...
    with patch.object(service.TestExecutor, '_validate_ssh_key', return_value=str(tmp_path / "fake-key")), \
         patch.object(service.TestExecutor, '_validate_python_path', return_value=Path("/usr/bin/python3")):
